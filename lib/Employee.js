...
class Employee {
    displayEmployees() {
        return db.promise().query(`SELECT employee.id, employee.first_name, employee.last_name, role.title AS title, department.name AS department, role.salary AS salary, CONCAT (employee2.first_name," ",employee2.last_name) AS manager
        FROM employee
        ...
        return db.promise().query(`SELECT CONCAT(first_name, " ", last_name) AS name FROM employee`)
            .then(([rows]) => {
                ...
                .then(({ employee }) => {
                    ...
                    return db.promise().query(`SELECT id FROM employee WHERE last_name = ?`, employeeArr[1])
                        .then(([row]) => row[0].id)
                })
            })
            .then(employee_id => {
            ...
            .then(({ employee_id, role_id }) => {
                db.query(`UPDATE employee SET role_id = ${role_id} WHERE id = ${employee_id}`, (err, row) => {
                    if(err) {
                        console.log(err);
                    }
                })
